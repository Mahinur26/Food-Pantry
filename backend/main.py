...
PROTECTED_PATHS = ["inventory", "chat"]
...
# Middleware to block access to protected routes
...
vertexai.init(project=PROJECT_ID, location=LOCATION, credentials=vertex_credentials)
...
async def add_inventory_from_image(
    ...
    try:
        ...
        expiration_date_normalized = (
            expiration_date if expiration_date and expiration_date.strip() else None
        )
